...
class Auth_model extends CI_Model {
    ...
    public function register($username, $password, $email, $additional_data = array(), $active = FALSE) {
        ...
        if(!isset($additional_data['group_id'])){ $additional_data['group_id'] = 3; } 
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, first_name, last_name, created_on, last_login, last_ip_address, avatar, gender, group_id')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                if(!isset($this->Settings->single_login)) { $this->Settings->single_login = 0; }
    ...
    public function login_remembered_user() {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login, last_ip_address, avatar, first_name, last_name, created_on, gender, group_id')
